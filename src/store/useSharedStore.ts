...
import { INITIAL_ORGANIZATIONS, SEED_BATCHES, SEED_FRAUD_ALERTS } from './seedData';
    ...
    (set) => ({
      ...
      seedIfEmpty: () => {
        ...
        set((state) => {
          ...
          if (!state.fraudAlerts || state.fraudAlerts.length === 0) {
            updates.fraudAlerts = SEED_FRAUD_ALERTS;
          }
